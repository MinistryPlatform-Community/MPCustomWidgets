...
You can build Stored Procedures in SQL Server that will return JSON data.  As of 2025.06.18 - Custom Widgets will honor this JSON and parse the object correctly passing on to your template for rendering.  This will super-cede the normal DataSet oriented data model.  There are some caveats to this working:
 ...
 - You **MUST** end the name of your Stored Procedure with (**_JSON**)
 - **ONE** object will be returned.  It may contain any number of nested objects, but only **ONE** object will be parsed and returned.  If there are more than 1 DataSets, it will be returned in the standard way.
 - Your Stored Procedure should be structured to use variables or a scalar subquery approach such that all JSON data is stored within a **single row**, in a **single column** with the column name **JsonResult**.
